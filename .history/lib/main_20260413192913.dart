    ...
    if (_formKey.currentState!.validate()) {
      try {
        ...
        if (result != null && result['token'] != null) {
          ...
          if (deletionSuccess['success'] == true) {
            setState(() {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                backgroundColor: ,
                content: Text(deletionSuccess['message'] ?? 'Conta excluida com sucesso.')),
            );
          } else {
            print('result: $result');
            setState(() {
  ...
  Future<Map<String, dynamic>?> getDeleteAccountInfo({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final data = jsonDecode(response.body);
        return {
          "error": data['error'] ?? 'Failed to authenticate. Please check your credentials and try again.',
        };
      }
    } catch (e) {
